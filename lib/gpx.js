...
function* render(ctx) {
  const gs = generatorStream();
  const out = stream(gs, { pretty: true });
  ...
  const $color = functions[target.trkColorFn];

  function outTextEl(tag, value) {
    out.elIfText(tag, value);
  }

  const $name = outTextEl.bind(undefined, 'name');
  ...
  const $desc = outTextEl.bind(undefined, 'desc');

  const $icon = outTextEl.bind(undefined, target.wptSymbolTag);
  ...
  function $rte_ext(route) {
    ...
    out.elIfText('gpxx:DisplayColor', getTrkColor(route) || 'Blue');
    out.end();
